...
DEFAULT_LISTEN_PORT="7126"
DEFAULT_PRUSA_HOST="http://prusa.local"
DEFAULT_PRUSA_STATUS_PATH="/api/v1/status"
DEFAULT_TIMEOUT="3.0"
DEFAULT_TTL="2.0"
DEFAULT_FALLBACK_TARGET="0.0"
DEFAULT_FALLBACK_CURRENT="0.0"
...
SERVICE_USER="$(prompt "Linux user for the service" "$CURRENT_USER")"
LISTEN_HOST="$(prompt "Bridge listen host" "$DEFAULT_LISTEN_HOST")"
LISTEN_PORT="$(prompt "Bridge listen port" "$DEFAULT_LISTEN_PORT")"
PRUSA_HOST="$(prompt "PrusaLink base URL" "$DEFAULT_PRUSA_HOST")"
PRUSA_STATUS_PATH="$(prompt "PrusaLink status path" "$DEFAULT_PRUSA_STATUS_PATH")"
PRUSA_AUTH_TYPE="$(prompt "PrusaLink auth type (digest/api_key/none)" "digest")"
REQUEST_TIMEOUT="$(prompt "PrusaLink timeout in seconds" "$DEFAULT_TIMEOUT")"
CACHE_TTL="$(prompt "Cache TTL in seconds" "$DEFAULT_TTL")"
FALLBACK_TARGET="$(prompt "Fallback bed target" "$DEFAULT_FALLBACK_TARGET")"
FALLBACK_CURRENT="$(prompt "Fallback bed current" "$DEFAULT_FALLBACK_CURRENT")"
LOG_LEVEL="$(prompt "Log level" "$DEFAULT_LOG_LEVEL")"
